...
function calcTurnings(startDate, actualDates = {}) {
  const turnings = [];
  let prevDate = new Date(startDate);
  for (let i = 0; i < TURNING_SCHEDULE.length; i++) {
    const t = TURNING_SCHEDULE[i];
    ...
    <div>
      ...
      {Object.entries(YARD_OPTIONS).map(([group, yards]) => (
        <div key={group} style={{ marginBottom: 10 }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: "#94a3b8", marginBottom: 5, letterSpacing: "0.05em" }}>{group}</div>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
            {yards.map(y => (
              <button key={y} onClick={() => onChange(y)}
                style={{ padding: "6px 12px", borderRadius: 8, border: "1.5px solid",
                  borderColor: value === y ? "#15803d" : "#e2e8f0",
                  background: value === y ? "#15803d" : "white",
                  color: value === y ? "white" : "#374151",
                  cursor: "pointer", fontSize: 13, fontWeight: value === y ? 700 : 400,
          ...
          return (
            <button key={n} onClick={() => onChange(lot)}
              style={{ width: 48, height: 48, borderRadius: 10, border: "1.5px solid",
                borderColor: value === lot ? "#15803d" : "#e2e8f0",
                background: value === lot ? "#15803d" : "white",
                color: value === lot ? "white" : "#374151",
                cursor: "pointer", fontSize: 14, fontWeight: value === lot ? 700 : 400,
                fontFamily: "inherit" }}>
              {lot}
            </button>
          );
...
export default function App() {
  ...
  const defaultForm = { type: "麦芽粕堆肥", lot: "ミ1", startDate: new Date().toISOString().split("T")[0], startYard: "", currentYard: "", note: "" };
  ...
  async function addBatch() {
    ...
    const nb = { ...form, id: Date.now(), done: false, actualDates: {}, actualNotes: {}, yardHistory: [], currentYard: form.startYard, createdBy: userName, createdAt: new Date().toISOString() };
  ...
  async function undoTurning(batchId, turnIndex) {
    if (!window.confirm(`切り返し${TURNING_SCHEDULE[turnIndex].label}の実施記録を取り消しますか？`)) return;
    const newBatches = batches.map(b => {
      if (b.id !== batchId) return b;
      const actualDates = { ...(b.actualDates||{}) };
      const actualNotes = { ...(b.actualNotes||{}) };
      ...
      delete actualDates[turnIndex];
      delete actualNotes[turnIndex];
      delete actualTemps[turnIndex];
      delete actualWaters[turnIndex];
      const lastH = [...yardHistory].reverse().find(h => h.turnIndex < turnIndex);
      const currentYard = lastH ? lastH.toYard : b.startYard;
      return { ...b, actualDates, actualNotes, actualTemps, actualWaters, yardHistory, currentYard };
    });
    await saveData(newBatches);
  }
  ...
  const allUpcoming = batches.filter(b=>!b.done).flatMap(b=>
    calcTurnings(b.startDate, b.actualDates||{})
  ...
  function getCalendarEvents() {
    ...
    batches.filter(b=>!b.done).forEach(b=>{
      ...
      calcTurnings(b.startDate, b.actualDates||{}).forEach((t,i)=>{
      ...
      <div>
        ...
        <div style={{ marginTop:24 }}>
          ...
          {allUpcoming.slice(0,8).map((t,i)=>{
            const diff=getDiff(t.date), s=STATUS[getStatus(t.date)], tc=TYPE_COLOR[t.batchType];
            return (
    ...
    <div style={{ fontFamily:"'Noto Sans JP','Hiragino Sans',sans-serif", background:"#f0fdf4", minHeight:"100vh" }}>
      ...
      {recordModal && (
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.55)", zIndex:100, display:"flex", alignItems:"flex-end", justifyContent:"center" }}
          ...
          <div style={{ background:"white", borderRadius:"20px 20px 0 0", padding:"28px 20px 32px", maxWidth:480, width:"100%", maxHeight:"90vh", overflowY:"auto" }}>
            ...
            <h3 style={{ margin:"0 0 4px", fontSize:18, color:"#166534" }}>
              ✅ 切り返し{TURNING_SCHEDULE[recordModal.turnIndex].label}を実施
            </h3>
            <p style={{ margin:"0 0 20px", fontSize:13, color:"#6b7280" }}>
              <strong>{recordModal.lot}</strong> — 実施日とヤードを記録します
            </p>
            ...
            {recordDate && (() => {
              const scheduled=new Date(recordModal.scheduledDate);
              const actual=new Date(recordDate);
              const diff=Math.round((actual-scheduled)/86400000);
              return diff!==0?(
            ...
            <div style={{ display:"flex", gap:8, marginBottom:20 }}>
              {[
                ...
                <button key={String(opt.val)} onClick={() => setRecordWater(recordWater === opt.val ? null : opt.val)}
                  style={{ flex:1, padding:"12px 0", borderRadius:10, border:"2px solid", borderColor: recordWater === opt.val ? opt.activeBorder : "#e2e8f0", background: recordWater === opt.val ? opt.activeBg : "white", color: recordWater === opt.val ? opt.activeColor : "#94a3b8", fontWeight: recordWater === opt.val ? 700 : 400, fontSize:14, cursor:"pointer", fontFamily:"inherit", transition:"all 0.15s" }}>
                  {opt.label}
                </button>
              ))}
            </div>
            <div style={{ marginBottom:16 }}>
              <YardSelector value={recordYard} onChange={setRecordYard} label={`切り返し後に入ったヤード（現在：${recordModal.currentYard||"未設定"}）`} />
              {!recordYard && <div style={{ fontSize:12, color:"#94a3b8", marginTop:6 }}>※ 変更がなければ選択不要</div>}
            </div>
            {recordDate && (() => {
              const batch=batches.find(b=>b.id===recordModal.batchId);
              if(!batch) return null;
              const newActual={...(batch.actualDates||{}),[recordModal.turnIndex]:recordDate};
              const upcoming=calcTurnings(batch.startDate,newActual).filter((t,i)=>i>recordModal.turnIndex&&!t.done);
              if(!upcoming.length) return null;
              return (
                <div style={{ marginBottom:16, padding:12, background:"#f8fafc", borderRadius:10, border:"1px solid #e2e8f0" }}>
                  <div style={{ fontSize:12, fontWeight:700, color:"#374151", marginBottom:6 }}>📅 更新後の予定日</div>
                  {upcoming.map((t,i)=>(
                    <div key={i} style={{ display:"flex", justifyContent:"space-between", fontSize:12, padding:"4px 0", borderBottom:i<upcoming.length-1?"1px solid #f1f5f9":"none" }}>
                      <span style={{ color:"#374151", fontWeight:600 }}>切り返し{t.label}</span>
        ...
        <div style={{ maxWidth:640, margin:"0 auto" }}>
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
            ...
            <div style={{ display:"flex", alignItems:"center", gap:8 }}>
              ...
              <button onClick={()=>{ localStorage.removeItem("compost-username"); setUserName(""); }} style={{ background:"rgba(255,255,255,0.2)", border:"none", borderRadius:8, color:"white", padding:"6px 10px", fontSize:12, cursor:"pointer", fontFamily:"inherit" }}>退出</button>
            </div>
          </div>
      ...
      <div style={{ background:"white", borderBottom:"1px solid #d1fae5", position:"sticky", top:0, zIndex:10 }}>
        <div style={{ maxWidth:640, margin:"0 auto", display:"flex" }}>
          {[{key:"schedule",label:"📋 一覧"},{key:"calendar",label:"📅 カレンダー"},{key:"add",label:"＋ 仕込み"}].map(nav=>(
            <button key={nav.key} onClick={()=>setView(nav.key)} style={{ flex:1, padding:"12px 0", border:"none", background:"transparent", fontWeight:view===nav.key?700:400, color:view===nav.key?"#15803d":"#6b7280", borderBottom:view===nav.key?"3px solid #15803d":"3px solid transparent", cursor:"pointer", fontSize:13, fontFamily:"inherit" }}>{nav.label}</button>
          ))}
        </div>
      </div>
      ...
      <div style={{ maxWidth:640, margin:"0 auto", padding:16 }}>
        ...
        {view==="schedule"&&(
          <div>
            <div style={{ display:"flex", background:"#e8f5e9", borderRadius:12, padding:4, marginBottom:14, gap:4 }}>
              {["進行中","完了"].map(s=>(
                <button key={s} onClick={()=>setFilterStatus(s)} style={{ flex:1, padding:"9px 0", borderRadius:9, border:"none", background:filterStatus===s?"white":"transparent", color:filterStatus===s?"#166534":"#6b7280", fontWeight:filterStatus===s?700:400, cursor:"pointer", fontSize:14, fontFamily:"inherit", boxShadow:filterStatus===s?"0 1px 4px rgba(0,0,0,0.1)":"none", transition:"all 0.2s" }}>
            ...
            <div style={{ display:"flex", gap:6, marginBottom:14, flexWrap:"wrap" }}>
              {["すべて",...COMPOST_TYPES].map(t=>(
                <button key={t} onClick={()=>setFilterType(t)} style={{ padding:"5px 12px", borderRadius:20, border:"2px solid", borderColor:filterType===t?"#15803d":"#d1fae5", background:filterType===t?"#15803d":"white", color:filterType===t?"white":"#15803d", cursor:"pointer", fontSize:11, fontWeight:filterType===t?700:400, fontFamily:"inherit" }}>{t==="すべて"?"すべて":TYPE_COLOR[t]?.shortLabel||t}</button>
              ))}
            </div>
            ...
            {filtered.map(batch=>{
              const turnings=calcTurnings(batch.startDate, batch.actualDates||{});
        ...
        {view==="add"&&(
          <div style={{ background:"white", borderRadius:16, padding:24, boxShadow:"0 2px 12px rgba(21,128,61,0.07)", border:"1.5px solid #d1fae5" }}>
            ...
            <div style={{ marginBottom:20 }}>
              <LotSelector type={form.type} value={form.lot} onChange={lot=>setForm({...form,lot})}/>
            </div>
            ...
            <div style={{ marginBottom:20 }}>
              <YardSelector value={form.startYard} onChange={v=>setForm({...form,startYard:v,currentYard:v})} label="仕込みヤード（初期ヤード）"/>
            </div>
            <label style={labelStyle}>メモ（任意）</label>
            ...
            {form.startDate&&(
              <div style={{ marginBottom:20, padding:14, background:"#f8fafc", borderRadius:10, border:"1px solid #e2e8f0" }}>
                <div style={{ fontSize:13, fontWeight:700, color:"#374151", marginBottom:8 }}>📅 切り返し予定日（初期）</div>
                {calcTurnings(form.startDate).map((t,i)=>(
                  <div key={i} style={{ display:"flex", justifyContent:"space-between", padding:"6px 0", borderBottom:i<6?"1px solid #f1f5f9":"none", fontSize:12 }}>
        ...
        {view==="detail"&&selectedBatch&&(()=>{
          const turnings=calcTurnings(selectedBatch.startDate, selectedBatch.actualDates||{});
          ...
          return (
            <div>
              ...
              <div style={{ background:"white", borderRadius:14, padding:18, marginBottom:12, boxShadow:"0 2px 12px rgba(21,128,61,0.07)", border:"1.5px solid #d1fae5" }}>
                ...
                <div style={{ marginTop:12, display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
                  <div onClick={()=>!selectedBatch.done && setEditingStartDate(editingStartDate===selectedBatch.id ? null : selectedBatch.id)}
                    style={{ background: editingStartDate===selectedBatch.id ? "#f0fdf4" : "#f8fafc", borderRadius:8, padding:"8px 12px", border: editingStartDate===selectedBatch.id ? "2px solid #15803d" : "2px solid transparent", cursor: selectedBatch.done ? "default" : "pointer", transition:"all 0.15s" }}>
                    ...
                    {editingStartDate===selectedBatch.id ? (
                      <div onClick={e=>e.stopPropagation()}>
                        ...
                        <div style={{ display:"flex", gap:6 }}>
                          ...
                          <button onClick={()=>{ const val=document.getElementById("startDateInput").value; if(val) updateStartDate(selectedBatch.id, val); }} style={{ flex:1, padding:"5px 0", borderRadius:6, border:"none", background:"#15803d", color:"white", fontSize:12, fontWeight:700, cursor:"pointer", fontFamily:"inherit" }}>保存</button>
                        </div>
                      </div>
                    ) : (
                      <div>
              ...
              <div style={{ fontSize:14, fontWeight:700, color:"#374151", marginBottom:8 }}>切り返しスケジュール</div>
              ...
              {turnings.map((t,i)=>{
                ...
                return (
                  <div key={i} onClick={()=>{
                    ...
                    else { setRecordDate(toDateStr(t.scheduledDate)); setRecordYard(selectedBatch.currentYard||""); setRecordModal({ batchId:selectedBatch.id, turnIndex:i, scheduledDate:t.scheduledDate, lot:selectedBatch.lot, currentYard:selectedBatch.currentYard }); }
                  }} style={{ background:s.bg, border:`1.5px solid ${s.border}`, borderRadius:12, padding:"14px 16px", marginBottom:8, cursor:"pointer", display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
                    <div style={{ flex:1 }}>
                      <div style={{ display:"flex", alignItems:"center", gap:8, flexWrap:"wrap" }}>
              ...
              {!selectedBatch.done&&(
                <button onClick={()=>markDone(selectedBatch.id)} disabled={saving} style={{ width:"100%", marginTop:16, padding:14, borderRadius:12, border:"2px solid #15803d", background:"white", color:"#15803d", fontSize:14, fontWeight:700, cursor:"pointer", fontFamily:"inherit" }}>
                  ✅ 堆肥完成・完了にする
                </button>
              )}
              <button onClick={()=>deleteBatch(selectedBatch.id)} disabled={saving} style={{ width:"100%", marginTop:8, padding:12, borderRadius:12, border:"2px solid #fca5a5", background:"white", color:"#dc2626", fontSize:13, fontWeight:600, cursor:"pointer", fontFamily:"inherit" }}>
                🗑 削除する
              </button>
            </div>
          );
